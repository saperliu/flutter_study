...
class _DropdownRoute<T> extends PopupRoute<_DropdownRouteResult<T>> {
  ...
  final List<DropdownMenuItemPlus<T>> items;
...
/// An item in a menu created by a [DropdownButtonPlus].
///
/// The type `T` is the type of the value the entry represents. All the entries
/// in a given menu must represent values with consistent types.
class DropdownMenuItemPlus<T> extends StatelessWidget {
  /// Creates an item for a dropdown menu.
  ///
  /// The [child] argument is required.
  const DropdownMenuItemPlus({
    Key key,
    this.value,
    @required this.child,
  }) : assert(child != null),
  ...
  ///
  /// Eventually returned in a call to [DropdownButtonPlus.onChanged].
  final T value;
...
/// An inherited widget that causes any descendant [DropdownButtonPlus]
/// widgets to not include their regular underline.
///
/// This is used by [DataTable] to remove the underline from any
/// [DropdownButtonPlus] widgets placed within material data tables, as
/// required by the material design specification.
class DropdownButtonHideUnderlinePlus extends InheritedWidget {
  /// Creates a [DropdownButtonHideUnderlinePlus]. A non-null [child] must
  /// be given.
  const DropdownButtonHideUnderlinePlus({
    Key key,
    @required Widget child,
  }) : assert(child != null),
        super(key: key, child: child);

  /// Returns whether the underline of [DropdownButtonPlus] widgets should
  /// be hidden.
  static bool at(BuildContext context) {
    return context.inheritFromWidgetOfExactType(DropdownButtonHideUnderlinePlus) != null;
  }

  @override
  bool updateShouldNotify(DropdownButtonHideUnderlinePlus oldWidget) => false;
}
...
/// Typically, an enum is used. Each [DropdownMenuItemPlus] in [items] must be
...
/// See also:
///
///  * [DropdownMenuItemPlus], the class used to represent the [items].
///  * [DropdownButtonHideUnderlinePlus], which prevents its descendant dropdown buttons
///    from displaying their underlines.
///  * [RaisedButton], [FlatButton], ordinary buttons that trigger a single action.
///  * <https://material.io/design/components/menus.html#dropdown-menu>
class DropdownButtonPlus<T> extends StatefulWidget {
  ...
  }) : assert(items == null || items.isEmpty || value == null || items.where((DropdownMenuItemPlus<T> item) => item.value == value).length == 1),
  ...
  /// The value of the currently selected [DropdownMenuItemPlus], or null if no
  ...
  @override
  _DropdownButtonPlusState<T> createState() => _DropdownButtonPlusState<T>();
}

class _DropdownButtonPlusState<T> extends State<DropdownButtonPlus<T>> with WidgetsBindingObserver {
  int _selectedIndex;
  ...
  @override
  void didUpdateWidget(DropdownButtonPlus<T> oldWidget) {
  ...
  void _updateSelectedIndex() {
    ...
    assert(widget.value == null ||
        widget.items.where((DropdownMenuItemPlus<T> item) => item.value == widget.value).length == 1);
    _selectedIndex = null;
  ...
  Widget build(BuildContext context) {
    ...
    if (widget.hint != null || (!_enabled && widget.disabledHint != null)) {
      ...
      _enabled ? widget.hint : DropdownMenuItemPlus<Widget>(child: widget.disabledHint ?? widget.hint);
    ...
    if (!DropdownButtonHideUnderlinePlus.at(context)) {
...
/// A convenience widget that wraps a [DropdownButtonPlus] in a [FormField].
class DropdownButtonFormField<T> extends FormField<T> {
  /// Creates a [DropdownButtonPlus] widget wrapped in an [InputDecorator] and
  /// [FormField].
  ///
  ...
  DropdownButtonFormField({
    Key key,
    T value,
    @required List<DropdownMenuItemPlus<T>> items,
    this.onChanged,
        ...
        super(
          ...
          builder: (FormFieldState<T> field) {
            ...
            return InputDecorator(
              ...
              isEmpty: value == null,
              child: DropdownButtonHideUnderlinePlus(
                child: DropdownButtonPlus<T>(
                  isDense: true,
                  value: value,
                  items: items,
                  hint: hint,
